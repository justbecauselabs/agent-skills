...
Always use argument and return validators for every Convex function. When you use [argument validation](/functions/validation.md), Convex will infer the types of your functions arguments automatically:
...
- Always `await` async calls (consider `@typescript-eslint/no-floating-promises`).
- In this repo, keep one exported Convex function per file.
- Public functions should enforce access control with `ctx.auth.getUserIdentity()` and avoid trusting spoofable args.
- Use `internal.*` references for `ctx.run*` and scheduler calls.
- Include the table name when calling `ctx.db.get/patch/replace/delete`.

If you want reusable types, annotate helper functions rather than skipping validators on Convex functions:
...
type SendMessageArgs = { body: string; author: string };
